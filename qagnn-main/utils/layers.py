...
import numpy as np
import math
from utils.utils import freeze_net, extract_query_rep


def average_entity_embeddings(batch_embedding, entity_mask):  # TODO 2 same function exist
...
class LSTM_MLP(nn.Module):
    """
    based on clutrr baseline's simpleEncoder
    """

    def __init__(self, sent_dim, fc_dim, fc_out_dim, n_fc_layer, dropouti, layer_norm):
        super().__init__()
        self.lstm = nn.LSTM(
            sent_dim,
            sent_dim,
        ...
        self.mlp = MLP(sent_dim * 3, fc_dim, fc_out_dim, n_fc_layer, dropout=dropouti, layer_norm=layer_norm)

    def forward(self, data, input_mask, node_type_ids, concept_ids, entity_mask):
        # ? data: (batch_size, seq_len 217, sent_dim)
        # inp_len = np.array(batch.inp_lengths)
        inp_len = input_mask.sum(dim=1).cpu().numpy()
        # sort
        ...
        inp_len_sorted = torch.from_numpy(inp_len_sorted).cpu()  # .to(data.device)
        ...
        outp, _ = pad_packed_sequence(outp, batch_first=True, total_length=data.size(1))  #!
        outp = outp.contiguous()
        outp_l = torch.zeros((data.size(0), data.size(1), outp.size(2))).to(device=outp.device, dtype=outp.dtype)
        ...
        outp_l = outp_l.contiguous()
        entid2lmemb = average_entity_embeddings(outp_l, entity_mask)
        query_rep = extract_query_rep(node_type_ids, concept_ids, entid2lmemb)

        # ?outp mean pool
        sent_len = torch.FloatTensor(inp_len.copy()).unsqueeze(1).to(outp_l.device)  # 2D Tensor (batch_size, 1)
        emb = torch.sum(outp_l, 1)  # (batch_size, hidden_dim(100)  )
        # BUG FIX: fails if batchsize is 1
        #!if emb.dim() > 2:
        #!    emb = emb.squeeze(0)
        emb = emb / sent_len.expand_as(emb)

        mlp_inp = torch.cat([query_rep, emb], dim=1)
...
class MLP(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size, num_layers, dropout, batch_norm=False, init_last_layer_bias_to_zero=False, layer_norm=False, activation="gelu"):
        super().__init__()
...
class TripleEncoder(nn.Module):
    def __init__(self, emb_dim, hidden_dim, input_p, output_p, hidden_p, num_layers, bidirectional=True, pad=False, concept_emb=None, relation_emb=None):
        super().__init__()
        if pad:
...
def masked_softmax(vector: torch.Tensor, mask: torch.Tensor, dim: int = -1, memory_efficient: bool = True, mask_fill_value: float = -1e32) -> torch.Tensor:
    """
...
class MatrixAttention(nn.Module):
    ...
    def forward(self, matrix_1: torch.Tensor, matrix_2: torch.Tensor) -> torch.Tensor:
        tiled_matrix_1 = matrix_1.unsqueeze(2).expand(matrix_1.size()[0], matrix_1.size()[1], matrix_2.size()[1], matrix_1.size()[2])
        tiled_matrix_2 = matrix_2.unsqueeze(1).expand(matrix_2.size()[0], matrix_1.size()[1], matrix_2.size()[1], matrix_2.size()[2])
...
class CustomizedEmbedding(nn.Module):
    def __init__(
        ...
        if LMentemb:
            concept_out_dim = concept_out_dim // 2
            self.lmvec2entemb = nn.Linear(sent_dim, concept_out_dim)
    ...
    def forward(self, index, contextualized_emb=None):
        ...
        """
        emb_from_embedding = self.emb(index) * self.scale
        if hasattr(self, "cpt_transform"):
            emb_from_embedding = self.activation(self.cpt_transform(emb_from_embedding))
        ...
        if contextualized_emb is not None:
            ...
            gathered_contextualized_emb = contextualized_emb.gather(1, index.unsqueeze(-1).expand(-1, -1, emb_dim))
            # Apply transform to the gathered contextualized embedding
            transformed_contextualized_emb = self.lmvec2entemb(gathered_contextualized_emb)
            # Concatenate embeddings from both sources
            final_embedding = torch.cat([emb_from_embedding, transformed_contextualized_emb], dim=-1)
            return final_embedding
        else:
            return emb_from_embedding
