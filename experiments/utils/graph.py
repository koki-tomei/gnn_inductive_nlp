...
def load_cpnet(cpnet_graph_path):
    ...
    import pickle
    with open(cpnet_graph_path, "rb") as fin:
        cpnet = pickle.load(fin)
...
def generate_adj_data_from_grounded_concepts__use_LM(grounded_path, cpnet_graph_path, cpnet_vocab_path, output_path, num_processes):
    ...
    この関数は、以下のものを保存する。
        (1) 隣接行列（それぞれ(R*N, N)個の疎行列の形式をとる）
        (2) 概念のID
        (3) ノードが質問概念であるかどうかを特定するためのqmask
        (4) ノードが回答概念であるかどうかを特定するamask
        (5) cid2score：概念IDとQA文脈での関連性スコアを対応付ける。
    をpythonのpickle形式で出力する。
